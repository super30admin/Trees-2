...
class Solution:
    ...
    def helper(self, root: Optional[TreeNode],currentNum:int)->None:
        ...
        #inorder traversal -> which happens after root.left and when root and corresponding currentNum is popped out of stack
        #if root.left == None and root.right == None:
         #   addition +=  currentNum * 10 + root.val
          #  return
        
        self.helper(root.right,currentNum * 10 + root.val)
        #postorder traversal -> which happens after root.right when root and corresponding currentNum is popped out of stack
        if root.left == None and root.right == None:
            addition +=  currentNum * 10 + root.val
            return
